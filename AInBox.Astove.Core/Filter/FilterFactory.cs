...
namespace AInBox.Astove.Core.Filter
{
    public static class FilterFactory
    {
        ...
        public async static Task<FilterOptions> GenerateFilterOptionsAsync(IComponentContext container, Type modelType, KeyValue parentId, IRequestFilter cmd)
        {
            var options = new FilterOptions { Filters = new List<PropertyValue>() };
            ...
            foreach (IFilter f in filters)
            {
                ...
                options.Filters.Add(await f.GetFilterOptionsAsync(container, cmd));
            }
        ...
        public static string GetWhereClause(Type modelType, List<FilterWebapiAttribute> attrs)
        {
            if (attrs == null || attrs.Count == 0)
                return null;

            var filters = new List<FilterCondition>();
            attrs.ForEach(a => filters.Add(a.CreateInstanceOf<FilterCondition>()));

            return JoinWhereClause(modelType, filters.ToArray());
        }
        ...
        public static object[] GetParameters(Type modelType, List<FilterWebapiAttribute> attrs)
        {
            if (attrs == null || attrs.Count == 0)
                return null;
            ...
            return GetParameters(modelType, filters.ToArray());
        }
        ...
        public async static Task<AInBox.Astove.Core.Options.Filter> GenerateFilterAsync(IComponentContext container, IRequestFilter cmd, Type modelType, KeyValue parentId)
        {
            FilterOptions options = await FilterFactory.GenerateFilterOptionsAsync(container, modelType, parentId, cmd);
            FilterCondition[] conditions = null;
            if (options != null)
                conditions = options.GetFilterConditions(cmd);

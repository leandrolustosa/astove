...
namespace AInBox.Astove.Core.Options
{
    public class FilterOptions
    {
        ...
        {
            PropertyValue pv = this.Filters.FirstOrDefault(f => f.Property.Equals(propertyName));
            var opValue = int.Parse(operatorValue);
            var op = pv.Operators.FirstOrDefault(o => o.Key == opValue);
            return (op != null) ? op.Value : EnumUtility.GetEnumText(typeof(MongoOperator), opValue);
        }
        ...
        private string GetDisplayValue(string filterType, string propertyName, string value)
        {
            ...
            if (pv.DomainValues != null)
            {
                ...
                if (pv.KeyKind == EnumDomainValues.KeyKind.String)
                    domainValue = pv.DomainValues.Cast<KeyValueString>().ToList().FirstOrDefault(d => d.Key == value).Value;
                else
                    domainValue = pv.DomainValues.Cast<KeyValue>().ToList().FirstOrDefault(d => d.Key == int.Parse(value)).Value;
                displayValue = domainValue;
            }
        ...
        private FilterCondition GetFilterCondition(PropertyValue value, object defaultValue)
        {
            ...
            IKeyValue[] domainValues = this.Filters.FirstOrDefault(f => f.Property.Equals(value.Property)).DomainValues;
            if (domainValues != null)
            {
                ...
                {
                    if (value.KeyKind == EnumDomainValues.KeyKind.String)
                        obj.DisplayValue = domainValues.Cast<KeyValueString>().ToList().FirstOrDefault(d => d.Key == Convert.ToString(obj.DefaultValue)).Value;
                    else
                        obj.DisplayValue = domainValues.Cast<KeyValue>().ToList().FirstOrDefault(d => d.Key == Convert.ToInt32(obj.DefaultValue)).Value;
                }
            }
            else
            {

...
namespace AInBox.Astove.Core.Util
{
    public class FileManager
    {
        ...
        public FileManager(HttpPostedFileBase file, string directory, string type)
        {
            this.File = file;
        ...
        public Dictionary<string, string> Upload()
        {
            ...
            try
            {
                ...
                var path = Path.Combine(System.Web.HttpContext.Current.Server.MapPath(string.Format("~/{0}/{1}", (this.Type.Equals("file")) ? "files" : "images", string.Join("/", this.Directory))), fileName);
                System.IO.FileInfo file = new System.IO.FileInfo(path);
                file.Directory.Create();

                var relativePath = Path.Combine(string.Format(@"\{0}\{1}", (this.Type.Equals("file")) ? "files" : "images", string.Join("\\", this.Directory)), fileName);
                File.SaveAs(path);

                result = new Dictionary<string, string>();
                result.Add("filename", (!string.IsNullOrEmpty(fileNameOriginal)) ? fileNameOriginal : fileName);

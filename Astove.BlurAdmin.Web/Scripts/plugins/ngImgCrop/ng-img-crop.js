...
(function () {
    ...
    crop.factory('cropArea', ['cropCanvas', function (CropCanvas) {
        ...
        CropArea.prototype._preventBoundaryCollision = function (size) {
            ...
            if (this.getType() === "circle" || this.getType() === "square" || this.getType() === "rectangle") {
                newSize = {
                    x: newSize.x,
                    y: newSize.y,
                    w: newSize.w,
                    h: newSize.w / this._aspectRatio
                };
            }
        ...
        return function (elCanvas, opts, events) {
            ...
            var resetCropHost = function () {
                if (image !== null) {
                    ...
                    if (size.w > size.h)
                        size.w = size.h * theArea.getAspectRatio();
                    else if (size.w < size.h)
                        size.h = size.w / theArea.getAspectRatio();
                    
                    theArea.setSize(size);
                    
                    theArea.setCenterPoint({ x: ctx.canvas.width / 2, y: 0 });

...
angular.module('AstoveApp').filter('cep', function () {
...
angular.module('AstoveApp').filter('cnpj', function () {
    return function (input) {
...
angular.module('AstoveApp').filter('cpf', function () {
    return function (input) {
        var str = input + '';
        str = str.replace(/\D/g, '');
        str = str.replace(/(\d{3})(\d)/, '$1.$2');
...
angular.module('AstoveApp').filter('realbrasileiro', function () {
    return function (input) {
        return 'R$ ' + formatReal(input);
    };
});
// Source: dist/.temp/brasil/filters/tel.js
angular.module('AstoveApp').filter('tel', function () {
    return function (input) {
        var str = input + '';
        str = str.replace(/\D/g, '');
        if (str.length === 11) {
